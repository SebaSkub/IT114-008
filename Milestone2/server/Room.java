...
public class Room implements AutoCloseable {
    ...
    private boolean processCommands(String message, ServerThread client) {
        ...
        try {
            if (message.startsWith(COMMAND_TRIGGER)) {
                ...
                wasCommand = true;
                switch (command) {
                    //Added two switch cases ROLL AND FLIP to process the trigger command
                    //ss365
                    //April 5th, 2023
                    case ROLL:
    ...
    protected synchronized void sendMessage(ServerThread sender, String message) {
        ...
        //Stacked Styling methods
        //ss365
        //April 5th, 2023
        //ITALIC
        if(message.startsWith("~")||(message.endsWith("~"))){   
                messageStyle = messageStyle.replaceAll("\\~(.*?)\\~", "<i>$1</i>");
                }
        //UNDERLINE
        if(message.startsWith("_")||(message.endsWith("_"))){   
                    messageStyle = messageStyle.replaceAll("\\_(.*?)\\_", "<u>$1</u>");
                    }
        //BOLD
        if(message.startsWith("*")||(message.endsWith("*"))){   
                    messageStyle = messageStyle.replaceAll("\\*(.*?)\\*", "<b>$1</b>");
                }
        //ss365
        //April 5th, 2023
        //RED
        if(message.startsWith("#r")||(message.endsWith("#"))){
                    messageStyle = messageStyle.replaceAll("\\#r(.*?)\\#", "<font color=\"#FF0000\">$1</font>");
                }
        //GREEN
        if(message.startsWith("#g")||(message.endsWith("#"))){
                    messageStyle = messageStyle.replaceAll("\\#g(.*?)\\#", "<font color=\"#00FF00\">$1</font>");
                    }
        //BLUE
        if(message.startsWith("#b")||(message.endsWith("#"))){
                        messageStyle = messageStyle.replaceAll("\\#b(.*?)\\#", "<font color=\"#0000FF\">$1</font>");
        ...
        while (iter.hasNext()) {
            ...
            boolean messageSent = client.sendMessage(from, messageStyle);
            if (!messageSent) {
